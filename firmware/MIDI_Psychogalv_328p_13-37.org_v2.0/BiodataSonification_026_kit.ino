...
int currScale = 0; //current scale, default Chrom //SETTING
int maxBrightness = 190; //SETTING
int channel = 1;  //setting channel to 11 or 12 often helps simply computer midi routing setups //SETTING
...
void readSettings() {
  ...
  EEPROM_readAnything(2, maxBrightness);
  EEPROM_readAnything(3, channel);
  EEPROM_readAnything(4, threshold);
}

void initializeEEPROM(short schema) {
  EEPROM_writeAnything(1, 0); // curScale default value
  EEPROM_writeAnything(2, 190); // maxBrightness default value
  EEPROM_writeAnything(3, 1); // MIDI channel default value
  EEPROM_writeAnything(4, 1.7f); // threshold default value
  schema = SCHEMA;
